...
const getUserTweets = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!(userTweets || userTweets.length === 0)) {
      throw new ApiError(
        500,
        `Can not fetch user ${user_Id} tweets at thid moment : try again later`
      );
    }
    ...
    throw new ApiError(
      500,
      error,
      "Could not fetch user tweets at thid moment"
    );
  }
}); //DONE!
...
const deleteTweet = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      error,
      "Something went wrong while deleting your tweet :Try again"
    );
  }
}); //DONE!
