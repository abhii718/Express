...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const subscriberIds = channelSubscriptions.map(
    ...
    throw new ApiError(
      500,
      error,
      "Something went wrong while getting subscribers :Please try again later"
    );
  }
}); //DONE!
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const channelIds = userSubscriptions.map(
