...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  try {
    const videoComments = await Comment.find({ video: video_Id })
      .skip((page - 1) * limit)
      .limit(limit)
      .exec();
...
const addComment = asyncHandler(async (req, res) => {
  ...
  if (!(video_Id || commentContent)) {
    ...
    throw new ApiError(
      404,
      "Invalid video_Id or you have not written any comment"
    );
  }
...
const deleteComment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!comment) {
      throw new ApiError(
        404,
        "comment not found : See if comment id is correct"
      );
    }
